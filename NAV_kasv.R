...
generate_animated_chart <- function() {
  ...
  # Loome baasjada kuu esimestest päevadest
  kuude_algused <- seq.Date(from=ymd("2018-01-01"), to=lopp_kp, by="1 month")
  
  # <--- MUUDATUS: Lisame kaadrite hulka ka kõige viimase kuupäeva --->
  # See tagab, et animatsiooni viimane kaader on identne staatilise pildiga.
  ...
  scaffold <- tidyr::expand_grid(seisuga_kp=kaadrite_kuupaevad, aasta=as.character(2017:year(lopp_kp)), name=c("LHVL","LHVXL","Tuleva","inflatsioon"))
  animeeritud_andmed_final <- scaffold %>% 
    left_join(animeeritud_andmed_raw, by=c("seisuga_kp","aasta","name")) %>%
    mutate(value=if_else(is.na(value),0,value), group_id=paste(aasta,name), name=factor(name,levels=c("LHVL","LHVXL","Tuleva","inflatsioon")), aasta=factor(aasta,levels=as.character(2017:year(lopp_kp))))
  
  anim_fps <- 10
  # Arvutame kaadrite arvu dünaamiliselt, et see vastaks üleminekute arvule
  nframes_total <- (length(kaadrite_kuupaevad) - 1) * anim_fps * 2 # Rohkem kaadreid sujuvamaks üleminekuks
  
  date_labels <- tibble(
    seisuga_kp = seq(from = min(kaadrite_kuupaevad), to = max(kaadrite_kuupaevad), length.out = nframes_total),
  ...
  # Dünaamiline kaadrite arvu ja pausi arvutamine
  total_transitions <- length(kaadrite_kuupaevad)
  dynamic_nframes <- total_transitions * anim_fps
  end_pause_frames <- anim_pause_sec * anim_fps
  
  gif_fail <- "aastane_tulu_animeeritud.gif"
  mp4_fail <- "aastane_tulu_animeeritud.mp4"
  
  message("Salvestan GIF-i...")
  ...
  ffmpeg_command <- paste0(
    ...
    " -vf 'scale=trunc(iw/2)*2:trunc(ih/2)*2' -y ",
    mp4_fail
  )
  
  system(ffmpeg_command)
  
  message("--- Animeeritud graafik valmis (GIF ja MP4) ---")
}
...
main <- function() {
  ...
  # Kuna MP4 luuakse GIF-ist, peame kontrollima ainult GIF-faili värskust.
