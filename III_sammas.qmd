...
puhasta_fondinimi <- function(nimi) {
  str_squish(str_replace_all(nimi, regex("pensionifond", ignore_case = TRUE), ""))
}

normaliseeri_fondi_voti <- function(nimi) {
  nimi %>%
    puhasta_fondinimi() %>%
    str_to_lower() %>%
    str_replace_all("väljumine piiratud", "") %>%
    str_replace_all("samba", "") %>%
    str_replace_all("iii", "") %>%
    str_replace_all("jätkusuutlik", "") %>%
    str_replace_all("kliimatuleviku", "") %>%
...
upsert_ajaloo_read <- function(vanad_andmed, uued_andmed, keys) {
  if (nrow(vanad_andmed) == 0) {
    return(uued_andmed)
  }

  vanad_andmed %>%
    anti_join(select(uued_andmed, all_of(keys)), by = keys) %>%
    bind_rows(uued_andmed) %>%
    arrange(!!!rlang::syms(keys))
}

loe_fondiloend_failist <- function(path) {
  read_csv(path, show_col_types = FALSE) %>%
    transmute(
      id = as.character(id),
      fond = puhasta_fondinimi(fond),
      indeks = ifelse(is.na(indeks), "", indeks)
    )
}

liida_indeksiinfo <- function(andmed, lookup) {
  lookup_clean <- lookup %>%
    transmute(
      fond = puhasta_fondinimi(fond),
      indeks = ifelse(is.na(indeks), "", indeks),
      fondi_voti = normaliseeri_fondi_voti(fond)
    ) %>%
    distinct(fondi_voti, .keep_all = TRUE)

  andmed %>%
    mutate(fondi_voti = normaliseeri_fondi_voti(Lühinimi)) %>%
    left_join(
      lookup_clean %>% transmute(fondi_voti, indeks_lookup = indeks, lookup_match = TRUE),
      by = "fondi_voti"
    ) %>%
    mutate(
      indeks = ifelse(is.na(indeks_lookup), "", indeks_lookup),
      lookup_match = ifelse(is.na(lookup_match), FALSE, lookup_match)
    ) %>%
    select(-indeks_lookup, -fondi_voti)
}
```
...
fallback_fondid <- loe_fondiloend_failist(seed_fondiloend)

# Eelistame jooksu käigus genereeritud fondiloendit, kuid puhtas kloonis
# või fetchi vea korral kasutame repoga kaasas olevat seed-snapshot'i.
fondid <- tryCatch({
  kandidaat <- loe_fondiloend_failist(runtime_fondiloend)

  if (nrow(kandidaat) < 5) {
    stop("genereeritud fondiloendis on liiga vähe ridu")
  }

  kandidaat
}, error = function(e) {
  message("Kasutan III samba seed-fondiloendit: ", e$message)
  fallback_fondid
})
...
if(!is.null(raw_maht) && nrow(raw_maht) > 0) {
  ...
  raw_maht_puhas <- raw_maht %>%
    mutate(
      Fond = puhasta_fondinimi(Fond),
      Lühinimi = puhasta_fondinimi(Lühinimi)
    )

  fondid_info <- liida_indeksiinfo(raw_maht_puhas, fondid)

  if (any(!fondid_info$lookup_match)) {
    message("CSV-põhine fondiloend ei katnud kõiki fonde, proovin fallback-vastendusi.")

    fallback_info <- liida_indeksiinfo(raw_maht_puhas, fallback_fondid) %>%
      transmute(
        Kuupäev,
        Lühinimi,
        fallback_indeks = indeks,
        fallback_lookup_match = lookup_match
      )

    fondid_info <- fondid_info %>%
      left_join(fallback_info, by = c("Kuupäev", "Lühinimi")) %>%
      mutate(
        indeks = ifelse(!lookup_match & fallback_lookup_match, fallback_indeks, indeks),
        lookup_match = lookup_match | fallback_lookup_match
      ) %>%
      select(-fallback_indeks, -fallback_lookup_match)
  }

  if (any(!fondid_info$lookup_match)) {
    message(
      "Hoiatus: indeksimärgistus jäi leidmata fondidele: ",
      paste(sort(unique(fondid_info$Lühinimi[!fondid_info$lookup_match])), collapse = ", ")
    )
  }

  fondid_info <- fondid_info %>% select(-lookup_match)
  ...
  if(file.exists(csv_ajalugu)) {
    ...
    kokku <- upsert_ajaloo_read(vanad_andmed, fondid_info, c("Kuupäev", "Lühinimi"))
    write_csv(kokku, csv_ajalugu)
    message("✓ III samba ajaloo fail sünkroniseeritud: ", nrow(fondid_info), " värsket rida töödeldud")
...
if(!is.null(raw_osakuomanikud) && nrow(raw_osakuomanikud) > 0) {
  ...
  if(file.exists(csv_investorid_ajalugu)) {
    ...
    kokku_investorid <- upsert_ajaloo_read(vanad_investorid, tanamased_investorid, c("Kuupäev", "Fond"))
    write_csv(kokku_investorid, csv_investorid_ajalugu)
    message("✓ Investorite ajaloo fail sünkroniseeritud: ", nrow(tanamased_investorid), " värsket rida töödeldud")
  } else {
    # Kui CSV ei eksisteeri, salvesta kõik andmed
...
if(nrow(fondid_info) > 0) {
  ...
  cat("Andmete laadimine ebaõnnestus. Palun kontrolli data/Pensionifondid_III_seed.csv faili või käivita fetch_iii_sammas_fondid.R, et luua värske Pensionifondid_III.csv.\n")
}
```
