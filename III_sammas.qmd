...
if(nrow(fondid_info) > 0) {
  ...
  if(nrow(indeksfondid) > 0) {
    indeksfondid_koos <- indeksfondid %>%
      ...
      mutate(
        indeks_maht_protsent = if_else(maht_kokku > 0, round(100 * maht / maht_kokku, 1), 0)
      )
  } else {
...
if(nrow(fondid_info) > 0) {
  ...
  fondid_turuosa <- fondid_info %>%
    group_by(Kuupäev) %>%
    mutate(
      turuosa_maht = if_else(sum(maht, na.rm = TRUE) > 0,
                            round(100 * maht / sum(maht, na.rm = TRUE), 1), 0)
    )
...
if(!is.null(raw_osakuomanikud) && nrow(raw_osakuomanikud) > 0) {
  kontode_turuosa_viimane <- raw_osakuomanikud %>%
    mutate(
      ...
      Fond = str_replace_all(Fond, regex("pensionifond", ignore_case = TRUE), ""),
      Fond = str_trim(Fond),
      turuosa_kontod = if_else(sum(osakuomanikud, na.rm = TRUE) > 0,
                              round(100 * osakuomanikud / sum(osakuomanikud, na.rm = TRUE), 1), 0)
    ) %>%
