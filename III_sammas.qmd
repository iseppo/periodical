...
```{r fondid_online}
# Hardcoded fallback, kui dünaamiliselt uuendatud fondiloend puudub.
fallback_fondid <- data.frame(
  id = c("55", "72", "87", "53", "41", "93", "43", "34",
         "90", "89", "33", "79", "42", "84", "32", "85", "81"),
  fond = c(
    "LHV Pensionifond Aktiivne III",
    "LHV Pensionifond Indeks III",
    "Luminor Jätkusuutlik Tulevik 50-55 Indeks Pensionifond",
    "Luminor Tulevik 16-50 Pensionifond",
    "Luminor Tulevik 55+ Pensionifond",
    "Luminor Tulevik Indeks Pensionifond",
...
# Eelistame fetch_iii_sammas_fondid.R loodud nimekirja, kuid jätame käsitsi kontrollitud fallback'i alles.
fondid <- tryCatch({
  kandidaat <- read_csv("Pensionifondid_III.csv", show_col_types = FALSE) %>%
    transmute(
      id = as.character(id),
      fond = puhasta_fondinimi(fond),
      indeks = ifelse(is.na(indeks), "", indeks)
  ...
  if (nrow(kandidaat) < 5) {
    stop("fondiloendis on liiga vähe ridu")
  }

  kandidaat
}, error = function(e) {
  message("Kasutan III samba fallback-fondiloendit: ", e$message)
  fallback_fondid
})
