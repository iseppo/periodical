...
if(!is.null(raw_maht) && nrow(raw_maht) > 0) {
  ...
  if(file.exists(csv_ajalugu)) {
    ...
    # Kui vanas failis puudub osakuomanike veerg, lisame selle
...
if(!is.null(raw_osakuomanikud) && nrow(raw_osakuomanikud) > 0) {
  ...
  tryCatch({
    download.file(investorid_andmebaas_url, csv_investorid_ajalugu, method = "auto", quiet = TRUE)
    if(file.exists(csv_investorid_ajalugu) && file.size(csv_investorid_ajalugu) > 0) {
  ...
  tanamased_investorid <- raw_osakuomanikud %>%
    mutate(
      Fond = str_replace_all(Fond, regex("pensionifond", ignore_case = TRUE), ""),
      Fond = str_trim(Fond),
      Kuupäev = as.Date(yesterday),  # yesterday on defineeritud real 140
