...
# UUENDUS: Laeme alla olemasoleva andmebaasi veebiserverist (kui see on olemas)
andmebaas_url <- "https://www.seppo.ai/tuleva/iii_sammas_ajalugu.csv"

# Püüame alla laadida olemasoleva andmebaasi
if(!file.exists(csv_ajalugu)) {
  message("Püüan laadida alla olemasoleva andmebaasi veebiserverist...")
  tryCatch({
    download.file(andmebaas_url, csv_ajalugu, method = "auto", quiet = TRUE)
    if(file.exists(csv_ajalugu) && file.size(csv_ajalugu) > 0) {
...
lae_mahud <- function() {
  # Kõik aktiivsed fondide ID-d (pärit veebilehe HTML-ist)
  # f[] parameetri abil saame kõigi fondide andmed
  all_fund_ids <- c(55, 72, 87, 53, 41, 43, 34, 90, 89, 33, 79, 42, 84, 32, 85, 81)
...
if(!is.null(raw_maht) && nrow(raw_maht) > 0) {
  ...
  fondid_info <- raw_maht %>%
    mutate(
      # Eemaldame "pensionifond" sõna nimedest
      Fond = str_replace_all(Fond, regex("pensionifond", ignore_case = TRUE), ""),
      Lühinimi = str_replace_all(Lühinimi, regex("pensionifond", ignore_case = TRUE), ""),
      # Puhastame üleliigsed tühikud
      Fond = str_trim(Fond),
      Lühinimi = str_trim(Lühinimi),
  ...
  # MÄRKUS: Osakuomanike arv (kontode arv) on saadaval AINULT viimase päeva kohta
  # Ajalugu ei ole võimalik alla laadida
  # Seetõttu EI lisata seda siia fondid_info tabelisse (mis sisaldab ajalugu)
  # Osakuomanike andmeid kasutatakse ainult viimase päeva graafikutes

  message("Laaditud fondide arv: ", length(unique(fondid_info$Fond)))
  message("ℹ Kontode arvu ajalugu ei ole saadaval - kasutatakse ainult viimase päeva andmeid")
...
if(nrow(indeksfondid) > 0) {
  ...
  colors_fondid <- rep(colors, ceiling(indeksfondid_arv / length(colors)))[1:indeksfondid_arv]
...
if(nrow(fondid_info) > 0) {
  # Arvutame kogusumma MAHU kohta (ainult maht, sest kontode arvu ei ole ajalooliselt)
...
if(nrow(fondid_info) > 0) {
  # Arvutame iga fondi turuosa MAHU järgi (kontode arvu ajalugu ei ole saadaval)
  ...
  # Arvutame iga kuupäeva kohta, mitu fondi on esindatud
  fondide_arv_kuupaeval <- fondid_turuosa %>%
    group_by(Kuupäev) %>%
    summarise(fondide_arv = n_distinct(Lühinimi), .groups = "drop")

  # Maksimum fondide arv (eeldame, et see on täielik andmestik)
  max_fondide_arv <- max(fondide_arv_kuupaeval$fondide_arv, na.rm = TRUE)

  # Viimane kuupäev, kus on maksimum fondide arv
  maksp2ev <- fondide_arv_kuupaeval %>%
    filter(fondide_arv == max_fondide_arv) %>%
    pull(Kuupäev) %>%
    max(na.rm = TRUE)

  message("Kasutame kuupäeva ", maksp2ev, " (", max_fondide_arv, " fondi andmetega)")
  ...
  base_colors <- c("#e60049", "#50e991", "#e6d800", "#9b19f5", "#ffa300",
                   "#dc0ab4", "#b3d4ff", "#00bfa0", "#ff6e54", "#444e86")
  # Kui fonde on rohkem kui baasvärvid, kordame neid
  colors_top <- rep(base_colors, ceiling(top_arv / length(base_colors)))[1:top_arv]
...
if(!is.null(raw_osakuomanikud) && nrow(raw_osakuomanikud) > 0) {
  kontode_turuosa_viimane <- raw_osakuomanikud %>%
    mutate(
      ...
      Fond = str_replace_all(Fond, regex("pensionifond", ignore_case = TRUE), ""),
      Fond = str_trim(Fond),
      turuosa_kontod = round(100 * osakuomanikud / sum(osakuomanikud, na.rm = TRUE), 1)
    ) %>%
    arrange(desc(osakuomanikud))

  # Värvid kontode graafikutele
  kontode_arv_viimane <- nrow(kontode_turuosa_viimane)
  kontode_base_colors <- c("#e60049", "#50e991", "#e6d800", "#9b19f5", "#ffa300",
                           "#dc0ab4", "#b3d4ff", "#00bfa0", "#ff6e54", "#444e86")
  colors_kontod <- rep(kontode_base_colors, ceiling(kontode_arv_viimane / length(kontode_base_colors)))[1:kontode_arv_viimane]
  names(colors_kontod) <- kontode_turuosa_viimane$Fond

  message("✓ Arvutatud turuosa kontode järgi viimase päeva kohta (",
          kontode_arv_viimane, " fondi)")
} else {
...
# MÄRKUS: See on duplikaat "Kontode arv" graafikuga
# Jätame selle sisse juhuks, kui kasutaja eelistab seda formaati
if(nrow(kontode_turuosa_viimane) > 0) {
  p <- kontode_turuosa_viimane %>%
    mutate(Fond = factor(Fond, levels = Fond)) %>%
    ggplot(aes(x = Fond, y = osakuomanikud, fill = Fond)) +
...
if(nrow(fondid_turuosa) > 0) {
  ...
  # Värvid
  top_arv_t <- nrow(top_turuosa)
  base_colors_t <- c("#e60049", "#50e991", "#e6d800", "#9b19f5", "#ffa300",
                      "#dc0ab4", "#b3d4ff", "#00bfa0", "#ff6e54", "#444e86")
  colors_turuosa <- rep(base_colors_t, ceiling(top_arv_t / length(base_colors_t)))[1:top_arv_t]
  names(colors_turuosa) <- top_turuosa$Lühinimi
