...
# Nimetame veerud ümber III samba kontekstis
if(!is.null(fondid_kontod) && nrow(fondid_kontod) > 0) {
...
if(!is.null(fondid_kontod) && !is.null(fondid_maht) &&
  ...
  # Lisame lühinimed (kui need on defineeritud)
  # III samba fondide lühinimed võivad olla erinevad, praegu kasutame fondi nime
  if(!"Lühinimi" %in% names(fondid_info)) {
    fondid_info <- fondid_info %>%
      mutate(Lühinimi = Fond)
  }
} else {
...
```{r eval=!is.na(andmed_alates)}
cat("Andmed:", andmed_alates, "--", andmed_kuni, "\n")
```
