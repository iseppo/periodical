...
if(nrow(fondid_top) > 0) {
  ...
  p <- fondid_top %>%
    ...
    labs(x = "kuupäev", y = "turuosa (%)",
         title = "Fondide turuosa kontode arvu järgi")
  ggplotly(p)
} else {
  cat("Andmed puuduvad")
}
...
if(nrow(fondid_top) > 0) {
  ...
  p <- fondid_top %>%
    ...
    labs(x = "kuupäev", y = "osakuomanike arv",
         title = "Fondide osakuomanike arv")
  ggplotly(p)
} else {
  cat("Andmed puuduvad")
}
...
if(nrow(fondid_turuosa) > 0) {
  ...
  # Võtame KÕIK fondid KONTODE ARVU järgi (ainult need, millel on andmed)
  top_kontod <- fondid_turuosa %>%
    filter(Kuupäev == maksp2ev, !is.na(`kontode arv`)) %>%
    arrange(desc(`kontode arv`))

  # Värvid
  top_arv_k <- nrow(top_kontod)
  base_colors_k <- c("#e60049", "#50e991", "#e6d800", "#9b19f5", "#ffa300",
                     "#dc0ab4", "#b3d4ff", "#00bfa0", "#ff6e54", "#444e86")
  colors_kontod <- rep(base_colors_k, ceiling(top_arv_k / length(base_colors_k)))[1:top_arv_k]
  names(colors_kontod) <- top_kontod$Lühinimi
  ...
  p <- top_kontod %>%
    ...
    labs(x = "", y = "osakuomanike arv",
         title = paste0("Fondide kontode arv (", maksp2ev, ")")) +
