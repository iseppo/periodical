...
p <- navid %>%
  ...
  geom_text(data = navid_end, aes(label = format(round(NAV,2), nsmall = 2), color = Fond), nudge_x = 35, show.legend = FALSE) +
...
sp500_tr_eur_df %>%
  ...
  geom_text(data = sp500_lastvalue, aes(label = paste0(round(SP500TR.Close), "€")), nudge_x = 35) +
  theme_ipsum_rc() +
...
Kuna seda inspireeris mind tegema mingi LHV fondide reklaam, siis võrdlen siin neid fonde, mida LHV agressiivselt soovitab, Tuleva indeksiga, kus ma ise olen.
