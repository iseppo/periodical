...
# --- Siltide paigutus joonte otsas ----------------------------------------
# Numbrid joonte otsas paigutame fikseeritud FÜÜSILISELE kaugusele, mitte
# kindla arvu päevade kaugusele. Kuna graafikud salvestatakse alati sama
# suurusega, annab kindel osakaal x-telje ulatusest püsiva kauguse (mm-tes).
# Varasem nudge_x = 45 (päeva) kahanes füüsiliselt seda enam, mida pikemaks
# aegrida kasvas, mistõttu numbrid nihkusid ajapikku joontele liiga lähedale.
SILDI_NIHE_OSAKAAL <- 0.02  # sildi ja joone otsa vahe osakaaluna x-ulatusest

# Sildi nihe andmeühikutes (päevades) antud kuupäevavahemiku kohta.
sildi_nihe_paevades <- function(kuupaevad) {
  as.numeric(max(kuupaevad) - min(kuupaevad)) * SILDI_NIHE_OSAKAAL
}
...
if (navid_ok) {
  ...
  navid_end <- navid_end %>%
    # factor() enne fct_reorder'i: forcats 1.0.1 viskab character-sisendi ja
    # NA väärtuse (nt fond, mille algne NAV oli 0) koosmõjul vea
    # "idx must contain one integer for each level of f".
    mutate(Fond = fct_reorder(factor(Fond), NAV)) %>%
  ...
  p <- navid %>%
    ...
    # Paremale jätame siltidele püsiva ruumi (osakaaluna teljest), et numbrid mahuksid.
    scale_x_date(date_labels = "%m/%Y", expand = expansion(mult = c(0.05, 0.08))) +
    geom_text(data = navid_end, aes(label = format(round(NAV,2), nsmall = 2, decimal.mark = ","), color = Fond), nudge_x = sildi_nihe_paevades(navid$Kuupäev), hjust = 0, show.legend = FALSE) +
  ...
  juhtimas <- tibble(Fond = "andmed puuduvad")
  max_kp <- ANDMETE_LOPP
  p <- ggplot() +
    annotate("text", x = 0.5, y = 0.5, label = "Pensionikeskuse NAV andmed pole kättesaadavad") +
    theme_void()
}
...
if (nrow(fondide_tasud) > 0) {
  p_tasud <- fondide_tasud %>%
    # factor() enne fct_reorder'i — vt selgitust navid_end juures ülalpool.
    ggplot(aes(x = fct_reorder(factor(fond), jooksvad_tasud_10k), y = jooksvad_tasud_10k)) +
...
if (yahoo_ok) {
  ...
  if (nrow(sp500_tr_eur_df) > 0) {
    ...
    p_summa <- sp500_tr_eur_df %>%
      ...
      geom_text(data = sp500_lastvalue, aes(label = paste0(round(SP500TR.Close), "€")), nudge_x = sildi_nihe_paevades(sp500_tr_eur_df$Date), hjust = 0) +
      scale_x_date(date_labels = "%m/%Y", expand = expansion(mult = c(0.05, 0.09))) +
      theme_ipsum_rc() +
