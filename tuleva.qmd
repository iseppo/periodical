...
navid <- read.delim(url, fileEncoding = "UTF-16", header = TRUE, dec = ",")
...
navid <- navid %>%
  ...
  mutate(NAV = if_else(first(NAV) != 0, NAV / first(NAV), NA_real_)) %>%
  ungroup()
...
sp500_ok <- tryCatch({
  getSymbols("^SP500TR", src = "yahoo", from = KIHLVEO_ALGUS, to = ANDMETE_LOPP)
  TRUE
}, error = function(e) {
...
eur_usd_ok <- tryCatch({
  getSymbols("EURUSD=X", src = "yahoo", from = KIHLVEO_ALGUS, to = ANDMETE_LOPP)
  TRUE
}, error = function(e) {
