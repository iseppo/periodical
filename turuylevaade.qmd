...
```{r include=FALSE}
plot_change <- function(period_months, data, variable){
  maksp2ev <- max(data$Kuupäev)
  if(period_months %% 12 == 0 && period_months >= 12){
    minp2ev <- maksp2ev %m-% years(period_months/12)
  } else {
    minp2ev <- maksp2ev %m-% months(period_months)
  }
  ...
  kuuinfo <- data %>%
    ...
    select(Lühinimi, value = !!sym(variable), Kuupäev) %>%
    pivot_wider(names_from = Kuupäev, values_from = value)
  ...
  label_pos <- kuuinfo_graaf %>% filter(muutus >= 0)
  label_neg <- kuuinfo_graaf %>% filter(muutus < 0)
  label_pos_tuleva <- kuuinfo_graaf %>% filter(muutus >= 0, Lühinimi == "TUK75")
  label_neg_tuleva <- kuuinfo_graaf %>% filter(muutus < 0, Lühinimi == "TUK75")
  ...
  p <- kuuinfo_graaf %>%
    ...
    theme_ft_rc() +
    labs(title = paste0(headline, minp2ev, " - ", maksp2ev), y = "", x = x_title)
  ...
  if(nrow(label_pos_tuleva) > 0) p <- p +
    geom_text(data = label_pos_tuleva, aes(label = label), nudge_x = kaugus * 0.06,
              color = "#00aeea")
  if(nrow(label_neg_tuleva) > 0) p <- p +
    geom_text(data = label_neg_tuleva, aes(label = label), nudge_x = kaugus * -0.06,
              color = "#00aeea")
...
```{r}
plot_change(1, indeksfondid, "investeerijate arv")
```
#### 3 kuud

```{r}
plot_change(3, indeksfondid, "investeerijate arv")
```


#### 6 kuud
```{r}
plot_change(6, indeksfondid, "investeerijate arv")
```


#### 12 kuud
```{r}
plot_change(12, indeksfondid, "investeerijate arv")
```
...
```{r}
plot_change(1, indeksfondid, "maht")
```
#### 3 kuud

```{r}
plot_change(3, indeksfondid, "maht")
```


#### 6 kuud
```{r}
plot_change(6, indeksfondid, "maht")
```


#### 12 kuud
```{r}
plot_change(12, indeksfondid, "maht")
```
