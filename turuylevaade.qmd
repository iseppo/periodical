...
# Eesti kümnenderaldaja (CI lülitab .Rprofile välja, seega seame siin)
options(OutDec = ",")
```
...
# Kui veebileht ei laadi, ei katkesta me renderdamist — kasutame allpool
# Pensionifondid.csv failis olevaid fondinimesid varuvariandina.
fondid_html <- tryCatch(
  read_html(url),
  error = function(e) {
    message("Pensionikeskus HTML ei laadnud: ", conditionMessage(e))
    NULL
  }
)

if (is.null(fondid_html)) {
  fund_info <- data.frame(ID = character(), Title = character(),
  ...
  # Extract the fund IDs from the URLs (ühe sammuga, lõpu kaldkriips valikuline)
  ...
  fund_info <- data.frame(ID = fund_ids, Title = fund_titles,
                          stringsAsFactors = FALSE) %>%
    filter(!is.na(ID), nchar(trimws(Title)) > 0)
}
...
if (nrow(fund_info) > 0) {
  # Värskendame fondinimed Pensionikeskusest (nimed võivad ajas muutuda).
  fondid <- fondid %>%
    select(-any_of("fond")) %>%
    left_join(select(fund_info, Title, ID), by = c("id" = "ID")) %>%
    rename(fond = Title)
} else {
  # Veebipäring ebaõnnestus — jääme Pensionifondid.csv failis olevate nimede juurde.
  message("Kasutan fondinimesid Pensionifondid.csv failist (veeb ei vastanud).")
}

puuduvad <- fund_info %>%
  filter(!ID %in% fondid$id)
puudu <- nrow(puuduvad) > 0
```
...
# this if clause doesn't seem to work as the max data is always from previous days I guess
  ...
  # Iga fondi laadimine on eraldi tryCatch'i sees: kui üks fond ei laadi (või
  # Pensionikeskus muudab veergude arvu), jätame selle fondi vahele, mitte ei
  # katkesta tervet raportit. colClasses = "character" loeb kõik veerud tekstina
  # — see EI ole positsiooniline (talub veergude lisandumist/kadumist) ja hoiab
  # veerutüübid fondide vahel ühtsena. Muidu annaks read.csv2 tüübituvastus
  # fonditi erineva tulemuse (tühi veerg -> logical, täisarvud -> integer),
  # mille peale uuem vctrs::vec_rbind keeldub future_map_dfr'i bind_rows'ist.
  # Arvtüübid teisendame allpool selgesõnaliselt.
  fondid_investorid <- furrr::future_map_dfr(fondid$id, function(i) {
    ...
    res <- tryCatch(
      ...
      error = function(e) {
        message("  fondi ", i, " investorite andmed ei laadinud: ",
                conditionMessage(e))
        NULL
      }
    )
    # Andmeteta vastuse jätame vahele, et raportisse ei satuks tühje ridu.
  ...
  fondid_maht <- furrr::future_map_dfr(fondid$id, function(i) {
    ...
    res <- tryCatch(
      ...
      error = function(e) {
        message("  fondi ", i, " mahu andmed ei laadinud: ",
                conditionMessage(e))
        NULL
      }
    )
    # Andmeteta vastuse jätame vahele (vt selgitust investorite ploki juures).
    if (is.null(res) || nrow(res) == 0) return(NULL)
    res
  })
...
# colClasses = "character" tõttu on kõik veerud tekstina — teisendame
# kasutatavad arvtüübid siin. teksti_arvuks talub koma-kümnendkohta ja
# tühikut tuhandeliste eraldajana (Eesti arvuvorming).
teksti_arvuks <- function(x) {
  as.numeric(gsub(",", ".", gsub("[[:space:]]", "", x)))
}

fondid_investorid <- fondid_investorid %>%
  rename("investeerijate arv" = Invest..arv, inv_arv_muutus = Muutus..) %>%
  mutate(`investeerijate arv` = as.integer(teksti_arvuks(`investeerijate arv`)))

fondid_maht <- fondid_maht %>%
  rename("maht" = Maht, mahu_muutus = Muutus..) %>%
  mutate(maht = teksti_arvuks(maht))

fondid_info <- full_join(fondid_maht, fondid_investorid, by = c("Kuupäev", "Lühinimi")) %>%
  select(-any_of("Fond.PIK")) %>%
...
indeks_pikk %>%
  ...
  write_csv(file = "koguturg_indeksfondid_osakaal.csv")
...
fondid_koos <- fondid_info %>%
  ...
  full_join(indeksfondid_koos, by = "Kuupäev")
...
p <- fondid_graaf_inv %>%
  ...
  geom_point(data = filter(fondid_graaf_inv, Kuupäev == max(Kuupäev)), size = 2, color = "orange") +
  geom_text(data = filter(fondid_graaf_inv, Kuupäev == max(Kuupäev)), aes(label = value_label), nudge_y = 30000, color = "orange") +
  geom_blank(aes(x = max(Kuupäev) + 100, y = min(0)))
...
graafikule2 <- tuleva_koos_volakiri %>%
  ...
  full_join(graafikule, by = "Kuupäev") %>%
...
# Selgitav tühi graafik olukorraks, kus andmeid ei jätku. Parem anda lugejale
# (ja logile) selge sõnum kui lasta kogu raporti renderdusel katkeda.
tyhi_graafik <- function(pealkiri, selgitus) {
  ggplot() +
    annotate("text", x = 0, y = 0, label = selgitus, colour = "grey70", size = 4.5) +
    theme_ft_rc() +
    theme(axis.text = element_blank(), axis.ticks = element_blank(),
          panel.grid = element_blank()) +
    labs(title = pealkiri, x = NULL, y = NULL)
}
...
make_change_plot <- function(data, months_back, var){
  ...
  pealkiri <- paste0(title_prefix, minp2ev, " - ", maksp2ev)

  aken <- data %>%
    ungroup() %>%
    filter(Kuupäev > minp2ev)

  # Muutuse arvutamiseks on vaja kahte otspunkti. Kui aknas on ainult üks
  # kuupäev (nt pikem andmekatkestus Pensionikeskuses), jätame graafiku vahele.
  if (nrow(aken) == 0) {
    message("make_change_plot (", var, ", ", months_back, " kuud): aknas ",
            minp2ev, " - ", maksp2ev, " ei ole ühtegi rida, jätan graafiku vahele")
    return(tyhi_graafik(pealkiri, "andmeid ei ole"))
  }
  otsad <- range(aken$Kuupäev, na.rm = TRUE)
  if (otsad[1] == otsad[2]) {
    message("make_change_plot (", var, ", ", months_back, " kuud): aknas on ainult ",
            "üks kuupäev (", otsad[1], "), jätan graafiku vahele")
    return(tyhi_graafik(pealkiri, "andmeid ei ole piisavalt"))
  }

  kuuinfo <- aken %>%
    filter(Kuupäev %in% otsad) %>%
    select(Lühinimi, !!sym(var), Kuupäev) %>%
    # Ühinenud fondid võivad jagada sama lühinime (vt case_when ülalpool). Ilma
    # values_fn'ita teeks pivot_wider neist list-veeru ja kogu raport kukuks.
    pivot_wider(
      names_from = Kuupäev,
      values_from = !!sym(var),
      names_prefix = "kp_",
      values_fn = function(x) if (all(is.na(x))) NA_real_ else sum(x, na.rm = TRUE)
    )

  # Nimetame otspunktid kuupäeva, mitte veeru asukoha järgi — pivot_wider'i
  # veergude järjekord sõltub ridade järjekorrast, mitte kuupäevast.
  eelmine_veerg <- paste0("kp_", otsad[1])
  tana_veerg <- paste0("kp_", otsad[2])
  if (!all(c(eelmine_veerg, tana_veerg) %in% names(kuuinfo))) {
    message("make_change_plot (", var, ", ", months_back, " kuud): otspunktide ",
            "veerud puuduvad, jätan graafiku vahele")
    return(tyhi_graafik(pealkiri, "andmeid ei ole piisavalt"))
  }
  kuuinfo <- kuuinfo %>%
    rename(eelmine_kuu = !!sym(eelmine_veerg), täna = !!sym(tana_veerg))

  if(var == "maht") {
    kuuinfo_graaf <- kuuinfo %>%
      mutate(muutus = round((täna - eelmine_kuu)/1000000, 1),
             muutus_pc = if_else(eelmine_kuu != 0, round(100 * muutus * 1000000 / eelmine_kuu), NA_real_))
  } else {
    kuuinfo_graaf <- kuuinfo %>%
      mutate(muutus = täna - eelmine_kuu,
             muutus_pc = if_else(eelmine_kuu != 0, round(100 * muutus / eelmine_kuu, 1), NA_real_))
  }

  # Fondid, kellel üks otspunkt puudub või on loetamatu (Pensionikeskuse
  # andmeauk), jätame graafikult välja — tulpa nagunii joonistada ei saa.
  puuduvad_fondid <- kuuinfo_graaf$Lühinimi[is.na(kuuinfo_graaf$muutus)]
  if (length(puuduvad_fondid) > 0) {
    message("make_change_plot (", var, ", ", months_back, " kuud): andmed puuduvad, ",
            "jätan graafikult välja: ", paste(puuduvad_fondid, collapse = ", "))
    kuuinfo_graaf <- filter(kuuinfo_graaf, !is.na(muutus))
  }
  if (nrow(kuuinfo_graaf) == 0) {
    message("make_change_plot (", var, ", ", months_back, " kuud): ühelgi fondil ",
            "ei ole mõlemat otspunkti, jätan graafiku vahele")
    return(tyhi_graafik(pealkiri, "andmeid ei ole"))
  }

  kuuinfo_graaf <- kuuinfo_graaf %>%
    # factor() on siin oluline: forcats 1.0.1 fct_reorder eemaldab NA-d
    # originaalvektorist (.f), aga võrdleb tulemust juba varem arvutatud
    # faktoritasemetega — character-sisendi puhul annab see vea
    # "idx must contain one integer for each level of f". Faktorina jäävad
    # tasemed alles ka siis, kui mõni rida välja langeb.
    mutate(Lühinimi = fct_reorder(factor(Lühinimi), muutus)) %>%
  ...
  p <- kuuinfo_graaf %>%
    ...
    theme_ft_rc() +
    labs(title = pealkiri, y = "", x = x_label)
...
data <- tryCatch(
  fromJSON(paste0("https://www.pensionikeskus.ee/ws/et/stats/fund-pev?=", now_unix)),
  error = function(e) stop("Pensionikeskus vahetuste JSON ei laadnud: ", conditionMessage(e))
)
...
jarjesta_fondid <- function(nimed, vaartus) {
  nimed_chr <- as.character(nimed)
  nimed_chr[is.na(nimed_chr) | nimed_chr == ""] <- "Tundmatu fond"
  factor(nimed_chr, levels = unique(nimed_chr[order(vaartus, na.last = TRUE)]))
}

ohutu_max <- function(x, default = 0) {
  vals <- x[is.finite(x)]
  if (length(vals) == 0) default else max(vals)
}

ohutu_min <- function(x, default = 0) {
  vals <- x[is.finite(x)]
  if (length(vals) == 0) default else min(vals)
}

get_fundname <- function(name){
  if(str_length(name) < 12) return(paste0("PIK: ", name))
  entry <- fundnames[[name]]
  if(is.null(entry)) return(paste0("Tundmatu: ", name))
  return(entry[[1]]$name)
}
...
fondinimed <- fondid_info %>%
  ...
  transmute(fond = Fond, lühinimi = Lühinimi) %>%
  distinct(fond, .keep_all = TRUE)
...
indeks_vahetus <- left_join(indeks_vahetus, fondinimed, by = "fond") %>%
  mutate(lühinimi = if_else(is.na(lühinimi) | lühinimi == "", fond, lühinimi))

# NB: count_to_fund sisaldab juba count_to_pik (liideti ülalpool),
# seega ei tohi neid uuesti kokku liita
vahetus_kokku <- sum(vahetus$count_to_fund)
vahetus_kokku_sum <- sum(vahetus$amount_from_fund)

indeksist_lahkunud <- sum(indeks_vahetus$count_to_fund)
indeksist_lahkunud_sum <- sum(indeks_vahetus$amount_to_fund)

indeksisse_lisandunud <- sum(indeks_vahetus$count_from_fund)
indeksisse_lisandunud_sum <- sum(indeks_vahetus$amount_from_fund)
...
Kokku vahetas fondi **`r format(vahetus_kokku, big.mark = " ", decimal.mark = ",")` inimest**, summas **`r format(round(vahetus_kokku_sum/1000000), big.mark = " ", decimal.mark = ",")` miljonit eurot** (keskmine summa **`r if(vahetus_kokku > 0) round(vahetus_kokku_sum/vahetus_kokku) else 0`€**).

Indeksfondidest lahkus **`r format(indeksist_lahkunud, big.mark = " ", decimal.mark = ",")` inimest**, summas **`r format(round(indeksist_lahkunud_sum/1000000), big.mark = " ", decimal.mark = ",")` miljonit eurot** (keskmine summa **`r if(indeksist_lahkunud > 0) format(round(indeksist_lahkunud_sum/indeksist_lahkunud), big.mark = " ", decimal.mark = ",") else "0"`€**).

Indeksfondidesse lisandus **`r format(indeksisse_lisandunud, big.mark = " ", decimal.mark = ",")` inimest**, summas **`r format(round(indeksisse_lisandunud_sum/1000000), big.mark = " ", decimal.mark = ",")` miljonit eurot** (keskmine summa **`r if(indeksisse_lisandunud > 0) format(round(indeksisse_lisandunud_sum/indeksisse_lisandunud), big.mark = " ", decimal.mark = ",") else "0"`€**).
...
graafikule <- indeks_vahetus %>%
  ...
  mutate(fond = jarjesta_fondid(lühinimi, huvitav)) %>%
...
p <- graafikule %>%
  ...
  geom_text(aes(label = label), nudge_x = ohutu_max(graafikule$huvitav * 0.06)) +
  geom_text(data = filter(graafikule, lühinimi == "Tuleva Akt"), 
            aes(label = label), nudge_x = ohutu_max(graafikule$huvitav * 0.06),
            color = "#00aeea") +
  theme_ft_rc()
p
```
...
```{r}
graafikule <- indeks_vahetus %>%
  mutate(huvitav = if_else(count_from_fund > 0, round(amount_from_fund/count_from_fund), 0L)) %>%
  mutate(fond = jarjesta_fondid(lühinimi, huvitav)) %>%
  mutate(label = paste0(huvitav, "€"))
...
```{r}
graafikule <- indeks_vahetus %>%
  mutate(huvitav = if_else(count_to_fund > 0, round(amount_to_fund/count_to_fund), 0L)) %>%
  mutate(fond = jarjesta_fondid(lühinimi, huvitav)) %>%
  mutate(label = paste0(huvitav, "€"))
...
if(nrow(label_pos) > 0) p <- p + geom_text(data = label_pos, aes(label = label), 
                                           nudge_x = ohutu_max(label_pos$huvitav * 0.06))
if(nrow(label_neg) > 0) p <- p + geom_text(data = label_neg, aes(label = label), 
                                           nudge_x = ohutu_min(graafikule$huvitav * 0.60 - 0.5),
                                           color = "brown3")
...
if(nrow(label_neg_tuleva) > 0) p <- p + geom_text(data = label_neg_tuleva, aes(label = label), 
                                           nudge_x = ohutu_min(graafikule$huvitav * 0.60),
                                           color = "#00aeea")
